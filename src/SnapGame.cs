...
namespace CardGames
{
    public class SnapGame
    {
        ...
        private static void HandleUserInput(Snap myGame)
        {
            ...
            // Flip next card when SPACE is pressed
            if (SwinGame.KeyTyped(KeyCode.vk_SPACE))
            {
                myGame.FlipNextCard();
            }

            // Only check hits if the game has started
            if (myGame.IsStarted)
            {
                // Both players hit at the same time -> dead heat (do nothing)
                if (SwinGame.KeyTyped(KeyCode.vk_LSHIFT) && SwinGame.KeyTyped(KeyCode.vk_RSHIFT))
                {
                    // TODO: add sound effects or message for dead heat
                }
                // Player 1 hit (Left Shift)
                else if (SwinGame.KeyTyped(KeyCode.vk_LSHIFT))
                {
                    myGame.PlayerHit(0);
                }
                // Player 2 hit (Right Shift)
